...
async function getToken() {
    const res = await credential.getToken("https://cognitiveservices.azure.com/.default");
    return res.token;
}
/**
 ...
 */
async function callGpt52(messages, temperature = 0.7, maxTokens) {
    ...
    if (usage) {
        console.error(`[gpt-5.2] tokens: ${usage.prompt_tokens} in / ${usage.completion_tokens} out / ${usage.total_tokens} total`);
    }
    return reply;
}
/**
 ...
 */
async function callO3Pro(input, maxTokens) {
    const token = await getToken();
    ...
    const usage = data.usage;
    if (usage) {
        console.error(`[o3-pro] tokens: ${usage.input_tokens} in / ${usage.output_tokens} out / ${usage.total_tokens} total`);
    }
    return reply;
}
/**
 ...
 *   400K context window.
 */
async function callCodex(input, maxTokens) {
    const token = await getToken();
    const url = `${AZURE_ENDPOINT}/openai/responses?api-version=${API_VERSION}`;
    // Convert ChatMessage[] to single string if needed
    let inputStr;
    ...
    const usage = data.usage;
    if (usage) {
        console.error(`[gpt-5.2-codex] tokens: ${usage.input_tokens} in / ${usage.output_tokens} out / ${usage.total_tokens} total`);
    }
    return reply;
}
...
// ── Tool: ask_o3_pro ───────────────────────────────────────────────────────────
server.tool("ask_o3_pro", "[STEP 3 of 3] Architecture review brain. Call this LAST, after ask_gpt52 and ask_codex. " +
    ...
    try {
        ...
        const messages = [systemMsg, ...history["o3-pro"]];
...
// ── Tool: ask_gpt52 ───────────────────────────────────────────────────────────
server.tool("ask_gpt52", "[STEP 1 of 3] Research synthesis brain. Call this FIRST to analyze raw search results, " +
    ...
    try {
        ...
        const messages = [systemMsg, ...history["gpt-5.2"]];
...
// ── Tool: ask_codex ────────────────────────────────────────────────────────────
server.tool("ask_codex", "[STEP 2 of 3] Code-level auditor brain. Call this AFTER ask_gpt52, BEFORE ask_o3_pro. " +
    ...
    try {
        ...
        const messages = [systemMsg, ...history["gpt-5.2-codex"]];
...
function isSecurityTask(message) {
...
async function safeBrainCall(brainName, fn) {
...
server.tool("ask_super", "Smart pipeline that auto-detects task type and routes to correct brains. " +
    ...
    if (isSecurityTask(message)) {
        ...
        const step2 = await safeBrainCall("gpt-5.2-codex", () => {
            const codexInput = `[System] You are an OWASP-certified application security auditor. ` +
        ...
        const step3 = await safeBrainCall("o3-pro", () => {
            const archInput = `[System] You are the Chief Security Architect. Review findings from the research team ` +
        ...
        addToHistory("super", "assistant", `[gpt-5.2]\n${step1.result}\n\n[codex]\n${step2.result}\n\n[o3-pro]\n${step3.result}`);
        return {
            content: [
                {
                    type: "text",
                    text: `# Security Audit Report (3-Brain Pipeline)\n` +
        ...
        const step2 = await safeBrainCall("gpt-5.2", () => {
            const implPrompt = [
                {
                    role: "system",
                    content: "You are an expert implementer. You received a plan from o3-pro (senior architect). " +
        ...
        console.error(`[ask_super] ===== PIPELINE COMPLETE: ${totalMs}ms | ${brainStatus} =====`);
        addToHistory("super", "user", message);
        addToHistory("super", "assistant", `[o3-pro plan]\n${step1.result}\n\n[gpt-5.2 implementation]\n${step2.result}`);
        return {
            content: [
                {
                    type: "text",
                    text: `# Super Agent Report (2-Brain Pipeline)\n` +
                        `> **Brain Status:** ${brainStatus} | Total: ${totalMs}ms\n\n` +
...
server.tool("ask_audit", "PREFERRED tool for security audits. Automatically runs all 3 brains in correct sequence: " +
    ...
    const step3 = await safeBrainCall("o3-pro", () => {
        const archInput = `[System] You are the Chief Security Architect. Review the findings from the research team ` +
            `and code auditor. Validate or challenge each finding. Identify anything they missed. ` +
...
server.tool("clear_history", "Clear conversation history for a model or all models.", {
    model: z
