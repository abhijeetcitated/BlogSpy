...
export function PricingModal() {
  ...
  const getVariantId = (planName: string) => {
    if (planName === "Pro") {
      return (
        (isYearly
  ...
  const handleCheckout = async (planName: string, credits: number) => {
    const variantId = getVariantId(planName)
    if (!variantId) {
      toast.error("Payment setup missing for this plan.")
      return
    }
    ...
    try {
      ...
      const validationMessage = result?.validationErrors?.variantId?._errors?.[0]
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (plan.name === "Agency") {
                          handleCheckout(plan.name, plan.credits)
                          return
                        }
                        handleCheckout(plan.name, plan.credits)
                      }}
                    ...
                    >
                      {isLoading && loadingPlan === plan.name ? "Redirecting..." : plan.cta}
                    </Button>
                  </div>
                )
              })}
