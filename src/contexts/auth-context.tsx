...
export function AuthProvider({ children, serverAccessToken = null }: AuthProviderProps) {
  const router = useRouter();
  const [user, setUser] = useState<SupabaseUser | DemoUser | null>(null);
  const [session, setSession] = useState<Session | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isDemoMode, setIsDemoMode] = useState(false);
  const serverAccessTokenRef = useRef<string | null>(serverAccessToken);

  useEffect(() => {
    serverAccessTokenRef.current = serverAccessToken ?? null;
  }, [serverAccessToken]);
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, currentSession) => {
        ...
        if (currentAccessToken !== serverAccessTokenRef.current) {
          serverAccessTokenRef.current = currentAccessToken;
          router.refresh(); // Sync server components with updated auth cookies
        }
